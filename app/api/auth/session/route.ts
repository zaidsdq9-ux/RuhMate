...
import { WELCOME_POINTS } from '@/lib/pricing';
...
export async function POST(req: NextRequest) {
  const rl = await rateLimit(req, 'auth:session');
  ...
  if (!userSnap.exists) {
    ...
    await userRef.set({
      ...
      status: 'active',
      // Explorer Access: every new member starts with free points (lib/pricing).
  ...
  res.cookies.set({
    ...
    sameSite: 'lax',
    path: '/',
    // No maxAge / expires → this is a SESSION cookie: the browser deletes it
    // when the browser is fully closed, forcing re-authentication on next
    // launch. SESSION_EXPIRES_MS still bounds the Firebase session cookie's
    // server-side validity (the upper limit if the browser is left open).
  });
  return res;
}
